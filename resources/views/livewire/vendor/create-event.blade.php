            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" x-show="show" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
                <form wire:submit="save">
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        ...
                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Banner Picture</label>
                            @if($oldPicture)
                                <div class="mb-2">
                                    <img src="{{  $oldPicture }}" alt="Current banner" class="h-32 w-full object-cover rounded">
                                </div>
                            @endif
                        ...
                        <div class="mb-4">
                            <label class="block text-gray-700 text-sm font-bold mb-2">Permit Document</label>
                            @if($oldPermitDocument)
                                <div class="mb-2">
                                    <a href="{{ asset('storage/' . $oldPermitDocument) }}" target="_blank" class="text-primary-600 hover:text-primary-700">View current document</a>
                                </div>
                            @endif
                            <input type="file" wire:model.live="permit_document" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                            @error('permit_document') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
